...
public class FieldValueProtoUtilsTests extends OpenSearchTestCase {
    ...
    public void testToProtoWithNestedMap() {
        ...
        assertEquals(
            "Nested string field should match",
            "nested value",
            nestedObjectMap.getFieldsOrThrow("nested_string").getString()
        );

        // Check nested integer field
        assertTrue("Nested integer field should exist", nestedObjectMap.containsFields("nested_integer"));
        assertTrue(
            "Nested integer field should have int32 value",
            nestedObjectMap.getFieldsOrThrow("nested_integer").hasInt32()
        );
        assertEquals(
            "Nested integer field should match",
            99,
            nestedObjectMap.getFieldsOrThrow("nested_integer").getInt32()
        );
    }
